  ...
  try {
    ...
    models.forEach(model => {
      ...
      modelCard.innerHTML = `
        ...
        <!-- Actions container -->
        <div class="card-actions">
          <button class="btn" onclick="viewModel('${fullPath}')">View in 3D</button>
          <button class="btn" onclick="downloadModel('${fullPath}', '${model.name}')">Download Model</button>
        </div>
      `;
      container.appendChild(modelCard);
    });
    ...
    document.getElementById('modelsContainer').innerHTML = `
      <p>Error loading models. Please check:</p>
      <ul>
        <li>Server is running</li>
        <li>Database connection</li>
        <li>Console for detailed errors</li>
      </ul>
      <button onclick="loadModels()" class="btn">Retry</button>
    `;
  }
}
